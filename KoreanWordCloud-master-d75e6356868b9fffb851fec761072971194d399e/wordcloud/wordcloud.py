...
from .korea_nlp import tokenize, noun_extract
FILE = os.path.dirname(__file__)
FONT_PATH = os.environ.get('FONT_PATH', os.path.join(FILE, 'H2GTRE.TTF'))
...
class WordCloud(object):
    ...
    def generate_from_text(self, text):
        ...
        self
        """
        # 한국어 일 때는 별도로 tokenization 하였다.
        if(self.lang == 'ko'):
            words = self.ko_process(text)
        else:
    ...
    def detect_language(self, text):
        ...
        if(lang == 'ko'):
            ...
            FONT_PATH = os.environ.get('FONT_PATH', os.path.join(FILE, 'msgothic.ttc'))
        elif(lang == 'zh-tw'):
            FONT_PATH = os.environ.get('FONT_PATH', os.path.join(FILE, 'mingliub.ttc'))
        elif(lang == 'zh-cn'):
            FONT_PATH = os.environ.get('FONT_PATH', os.path.join(FILE, 'ariblk.ttf'))
        elif(lang == 'es'):
            FONT_PATH = os.environ.get('FONT_PATH', os.path.join(FILE, 'ariblk.ttf'))
        elif(lang=='el'):
            FONT_PATH = os.environ.get('FONT_PATH', os.path.join(FILE, 'arial.ttf'))
        elif(lang=='th'):
            FONT_PATH = os.environ.get('FONT_PATH', os.path.join(FILE, 'LeelawUI.ttf'))
        else:
            FONT_PATH = os.environ.get('FONT_PATH', os.path.join(FILE, 'msgothic.ttc'))
    ...
    def ko_process(self, text):
        """한국어를 사용할 때의 process_text 메소드를 따로 만들었습니다.
        ...
        Notes
        -----
        한국어 Tokenization을 위해 KonlPy twitter API를 사용했습니다.
        """
        ...
        words = noun_extract(text, scores)
        # remove stopwords
        words = [word for word in words if word.lower() not in stopwords]
